...
def _create_behaviour_analysis_tool(behaviour_db: dict[str, Any]) -> Tool:
    ...
    for principle in principles:
        ...
        principle_descriptions.append(
            f"- {principle_id}: {name} - {description}"
        )
    ...
    parameters_schema = Schema(
        ...
        required=["principle_id", "reason", "intervention_suggestions", "triggers_matched"],
    )
        ...
        if (
            ...
            for part in response.candidates[0].content.parts:
                if hasattr(part, "function_call") and part.function_call:
                    ...
                    logger.info(
                        f"LLM recommended principle: {args.get('principle_id', 'unknown')}"
                    )
                    logger.info(f"LLM reasoning: {args.get('reason', 'N/A')}")
                    logger.info(
                        f"LLM interventions: {args.get('intervention_suggestions', [])}"
                    )
                    logger.info(
                        f"LLM matched triggers: {args.get('triggers_matched', [])}"
                    )
                    ...
                    if not any(
                        ...
                        logger.warning(
                            f"LLM suggested unknown principle: {result['detected_principle_id']}"
                        )
                        return None
...
def _build_memory_context(user_memory: Any) -> str:
    ...
    if hasattr(user_memory, "streaks") and user_memory.streaks:
        ...
        context_parts.append(
            f"Streaks: {active_streaks}/{streak_count} active"
        )
